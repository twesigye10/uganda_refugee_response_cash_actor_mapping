...
server <- function(input, output, session) {
    ...
    els_filter_cash_data <- function(input_df){
        # defaultly display all data from all districts, years and all quarters
        if (input$els_yearperiod == "All" & input$els_quarterperiod == "All"){
            input_df
        }else if(input$els_yearperiod == "All" & input$els_quarterperiod != "All"){
            input_df %>%
                filter(Quarter == input$els_quarterperiod )
        }else if(input$els_yearperiod != "All" & input$els_quarterperiod == "All"){
            input_df %>%
                filter(Year == input$els_yearperiod)
        } else{
            input_df %>%
                filter(Year == input$els_yearperiod, Quarter == input$els_quarterperiod )
        }
    }
    ...
    # filter cash data by district
    els_filter_cash_data_by_district <- function(input_df, input_district_click){
        input_df %>% 
            filter(location_district == input_district_click )
    }
