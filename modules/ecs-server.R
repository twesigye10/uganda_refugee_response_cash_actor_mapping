...
# get year
ecsYearValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$ecs_yearperiod})  )
  })
}
# get quarter
ecsQuarterValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$ecs_quarterperiod})  )
  })
}
# reset map
ecsResetMapServer <- function(id){
  moduleServer(id, function(input, output, session){
    return(
      input$ecs_mapreset
    )
...
ecsClickedDistrictValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    click = input$ecs_map_shape_click
    click_district <- click$id
    return( click_district )
  })
}
# donut chart module ------------------------------------------------------
ecsDonutChartCashBeneficiary <- function(id, input_data, input_field_group,
                                      input_field_analysis, input_title, input_beneficiary_vector){
  moduleServer(id, function(input, output, session){
    output$ecs_hhreceivingcash <-  renderBillboarder({
...
ecsLineChartTotalCashQuarter <- function(id, input_data, input_field_analysis, input_field_year, 
                                      input_field_quarter, input_field_select_Month, 
                                       input_title){
  moduleServer(id, function(input, output, session){
    output$ecs_plotcashquarter <-  renderHighchart({
      input_data %>%
...
ecsBarChartDeliveryMechanism <- function(id, input_data, input_field_group, input_field_analysis,  
                                      input_title){
  moduleServer(id, function(input, output, session){
    output$ecs_plotdeliverymechanism <-  renderHighchart({
      input_data %>%
...
ecsBarChartCashByPartner <- function(id, input_data, input_field_group, input_field_analysis,  
                                  input_title){
  moduleServer(id, function(input, output, session){
    output$ecs_plotcashpartner <-  renderHighchart({
      input_data %>%
        group_by({{input_field_group}} ) %>%
        summarise(
...
ecsTextSelectedDistrict <- function(id, input_text){
  moduleServer(id, function(input, output, session){
    output$ecs_selecteddistrict <- renderText({
...
# ecs default map module ------------------------------------------------------
ecsDefaultMap <- function(id){
  moduleServer(id, function(input, output, session){
    output$ecs_map  <-  renderLeaflet({
      leaflet(options = leafletOptions(zoomSnap = 0.25, zoomDelta=0.25)) %>% 
        ...
        addEasyButton(easyButton(
          ...
          onClick=JS("function(btn, ecs_map){ ecs_map.setView(new L.LatLng(1.3733,32.2903), 6.75) }")))
    })
  })
}
# update quarter module ------------------------------------------------------
ecsUpdateQuarter <- function(id, input_quarter_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "ecs_quarterperiod", 
...
# update year module ------------------------------------------------------
ecsUpdateYear <- function(id, input_year_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "ecs_yearperiod", 
                      label = "Select Year", 
