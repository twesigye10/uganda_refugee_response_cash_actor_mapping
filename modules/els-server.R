...
# donut chart module ------------------------------------------------------
elsDonutChartCashBeneficiary <- function(id, input_data){
  moduleServer(id, function(input, output, session){
    output$els_hhreceivingcash <-  renderBillboarder({
      df_billb_data <- input_data %>% 
        group_by(select_beneficiary_type) %>% 
        summarise(
          cash_assistance_by_beneficiary_type = sum(total_cash_value_of_cash_for_work_ugx, na.rm = T)
        ) 
...
elsLineChartTotalCashQuarter <- function(id, input_data){
  moduleServer(id, function(input, output, session){
...
elsBarChartDeliveryMechanism <- function(id, input_data){
  moduleServer(id, function(input, output, session){
    output$els_plotdeliverymechanism <-  renderHighchart({
      input_data %>%
        group_by(delivery_mechanism) %>%
        summarise(
          cash_transfer_by_delivery_mechanism = sum(total_cash_value_of_cash_for_work_ugx, na.rm = T)
        ) %>%
...
elsBarChartCashByPartner <- function(id, input_data){
  moduleServer(id, function(input, output, session){
