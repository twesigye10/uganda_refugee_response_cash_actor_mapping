...
# get year
shlYearValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$shl_yearperiod})  )
  })
}
# get quarter
shlQuarterValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$shl_quarterperiod})  )
  })
}
# reset map
shlResetMapServer <- function(id){
  moduleServer(id, function(input, output, session){
    return(
      input$shl_mapreset
    )
...
shlClickedDistrictValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    click = input$shl_map_shape_click
    click_district <- click$id
    return( click_district )
  })
}
# donut chart module ------------------------------------------------------
shlDonutChartCashBeneficiary <- function(id, input_data, input_field_group,
                                      input_field_analysis, input_title, input_beneficiary_vector){
  moduleServer(id, function(input, output, session){
    output$shl_hhreceivingcash <-  renderBillboarder({
...
shlLineChartTotalCashQuarter <- function(id, input_data, input_field_analysis, input_field_year, 
                                      input_field_quarter, input_field_select_Month, 
                                       input_title){
  moduleServer(id, function(input, output, session){
    output$shl_plotcashquarter <-  renderHighchart({
      input_data %>%
...
shlBarChartDeliveryMechanism <- function(id, input_data, input_field_group, input_field_analysis,  
                                      input_title){
  moduleServer(id, function(input, output, session){
    output$shl_plotdeliverymechanism <-  renderHighchart({
      input_data %>%
...
shlBarChartCashByPartner <- function(id, input_data, input_field_group, input_field_analysis,  
                                  input_title){
  moduleServer(id, function(input, output, session){
    output$shl_plotcashpartner <-  renderHighchart({
      input_data %>%
        group_by({{input_field_group}} ) %>%
        summarise(
...
shlTextSelectedDistrict <- function(id, input_text){
  moduleServer(id, function(input, output, session){
    output$shl_selecteddistrict <- renderText({
...
# shl default map module ------------------------------------------------------
shlDefaultMap <- function(id){
  moduleServer(id, function(input, output, session){
    output$shl_map  <-  renderLeaflet({
      leaflet(options = leafletOptions(zoomSnap = 0.25, zoomDelta=0.25)) %>% 
        ...
        addEasyButton(easyButton(
          ...
          onClick=JS("function(btn, shl_map){ shl_map.setView(new L.LatLng(1.3733,32.2903), 6.75) }")))
    })
  })
}
# update quarter module ------------------------------------------------------
shlUpdateQuarter <- function(id, input_quarter_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "shl_quarterperiod", 
...
# update year module ------------------------------------------------------
shlUpdateYear <- function(id, input_year_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "shl_yearperiod", 
                      label = "Select Year", 
