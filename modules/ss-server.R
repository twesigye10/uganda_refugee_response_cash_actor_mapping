...
# get year
ssYearValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$ss_yearperiod})  )
  })
}
# get quarter
ssQuarterValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$ss_quarterperiod})  )
  })
}
# reset map
ssResetMapServer <- function(id){
  moduleServer(id, function(input, output, session){
    return(
      input$ss_mapreset
    )
...
ssClickedDistrictValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    click = input$ss_map_shape_click
    click_district <- click$id
    return( click_district )
  })
}
# donut chart module ------------------------------------------------------
ssDonutChartCashBeneficiary <- function(id, input_data, input_field_group,
                                      input_field_analysis, input_title, input_beneficiary_vector){
  moduleServer(id, function(input, output, session){
    output$ss_hhreceivingcash <-  renderBillboarder({
...
ssLineChartTotalCashQuarter <- function(id, input_data, input_field_analysis, input_field_year, 
                                      input_field_quarter, input_field_select_Month, 
                                       input_title){
  moduleServer(id, function(input, output, session){
    output$ss_plotcashquarter <-  renderHighchart({
      input_data %>%
...
ssBarChartDeliveryMechanism <- function(id, input_data, input_field_group, input_field_analysis,  
                                      input_title){
  moduleServer(id, function(input, output, session){
    output$ss_plotdeliverymechanism <-  renderHighchart({
      input_data %>%
...
ssBarChartCashByPartner <- function(id, input_data, input_field_group, input_field_analysis,  
                                  input_title){
  moduleServer(id, function(input, output, session){
    output$ss_plotcashpartner <-  renderHighchart({
      input_data %>%
        group_by({{input_field_group}} ) %>%
        summarise(
...
ssTextSelectedDistrict <- function(id, input_text){
  moduleServer(id, function(input, output, session){
    output$ss_selecteddistrict <- renderText({
...
# ss default map module ------------------------------------------------------
ssDefaultMap <- function(id){
  moduleServer(id, function(input, output, session){
    output$ss_map  <-  renderLeaflet({
      leaflet(options = leafletOptions(zoomSnap = 0.25, zoomDelta=0.25)) %>% 
        ...
        addEasyButton(easyButton(
          ...
          onClick=JS("function(btn, ss_map){ ss_map.setView(new L.LatLng(1.3733,32.2903), 7.25) }")))
    })
  })
}
# ss dynamic map layer module ------------------------------------------------------
ssCreatingMap <- function(id, input_data){
  moduleServer(id, function(input, output, session){
    ...
    proxy = leafletProxy("ss_map", data = input_data)  
    proxy %>% 
      clearControls() %>% 
      addPolygons(
        color = "white",
...
# ss dynamic map labels module ------------------------------------------------------
ssMapLabels <- function(id, input_data){
  moduleServer(id, function(input, output, session){
    # label districts in the map
    labels_district <- ~sprintf(
      "<strong>%s</strong>",
...
ssUpdateQuarter <- function(id, input_quarter_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "ss_quarterperiod", 
...
# update year module ------------------------------------------------------
ssUpdateYear <- function(id, input_year_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "ss_yearperiod", 
                      label = "Select Year", 
