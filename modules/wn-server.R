...
# get year
wnYearValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$wn_yearperiod})  )
  })
}
# get quarter
wnQuarterValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    return( reactive({input$wn_quarterperiod})  )
  })
}
# reset map
wnResetMapServer <- function(id){
  moduleServer(id, function(input, output, session){
    return(
      input$wn_mapreset
    )
...
wnClickedDistrictValueServer <- function(id){
  moduleServer(id, function(input, output, session){
    click = input$wn_map_shape_click
    click_district <- click$id
    return( click_district )
  })
}
# donut chart module ------------------------------------------------------
wnDonutChartCashBeneficiary <- function(id, input_data, input_field_group,
                                      input_field_analysis, input_title, input_beneficiary_vector){
  moduleServer(id, function(input, output, session){
    output$wn_hhreceivingcash <-  renderBillboarder({
...
wnLineChartTotalCashQuarter <- function(id, input_data, input_field_analysis, input_field_year, 
                                      input_field_quarter, input_field_select_Month, 
                                       input_title){
  moduleServer(id, function(input, output, session){
    output$wn_plotcashquarter <-  renderHighchart({
      input_data %>%
...
wnBarChartDeliveryMechanism <- function(id, input_data, input_field_group, input_field_analysis,  
                                      input_title){
  moduleServer(id, function(input, output, session){
    output$wn_plotdeliverymechanism <-  renderHighchart({
      input_data %>%
...
wnBarChartCashByPartner <- function(id, input_data, input_field_group, input_field_analysis,  
                                  input_title){
  moduleServer(id, function(input, output, session){
    output$wn_plotcashpartner <-  renderHighchart({
      input_data %>%
        group_by({{input_field_group}} ) %>%
        summarise(
...
wnTextSelectedDistrict <- function(id, input_text){
  moduleServer(id, function(input, output, session){
    output$wn_selecteddistrict <- renderText({
...
# wn default map module ------------------------------------------------------
wnDefaultMap <- function(id){
  moduleServer(id, function(input, output, session){
    output$wn_map  <-  renderLeaflet({
      leaflet(options = leafletOptions(zoomSnap = 0.25, zoomDelta=0.25)) %>% 
        ...
        addEasyButton(easyButton(
          ...
          onClick=JS("function(btn, wn_map){ wn_map.setView(new L.LatLng(1.3733,32.2903), 7.25) }")))
    })
  })
}
# wn dynamic map layer module ------------------------------------------------------
wnCreatingMap <- function(id, input_data){
  moduleServer(id, function(input, output, session){
    ...
    proxy = leafletProxy("wn_map", data = input_data)  
    proxy %>% 
      clearControls() %>% 
      addPolygons(
        color = "white",
...
# wn dynamic map labels module ------------------------------------------------------
wnMapLabels <- function(id, input_data){
  moduleServer(id, function(input, output, session){
    # label districts in the map
    labels_district <- ~sprintf(
      "<strong>%s</strong>",
...
wnUpdateQuarter <- function(id, input_quarter_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "wn_quarterperiod", 
...
# update year module ------------------------------------------------------
wnUpdateYear <- function(id, input_year_choices, input_selected){
  moduleServer(id, function(input, output, session){
    updateSelectInput(session, "wn_yearperiod", 
                      label = "Select Year", 
